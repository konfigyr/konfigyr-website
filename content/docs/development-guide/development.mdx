...
After cloning the repository, developers start by configuring the local development stack. Running the Docker Compose
file launches PostgreSQL and any supporting services required by the IdP and API.
...
You can start by copying the contents of the `application-local.yml.template` located in `/src/main/resources`
